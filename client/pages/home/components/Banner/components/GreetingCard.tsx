...
const GreetingCard: React.FC<GreetingCardProps> = ({ name, image }) => {
  ...
  return (
    <div
      className="bg-[#303030] hover:bg-[#454545] transition duration-300 flex h-24 tracking-wider font-semibold cursor-pointer w-full rounded-md overflow-hidden"
      onMouseEnter={() => setShowPlayButton(true)}
      onMouseLeave={() => setShowPlayButton(false)}
    >
      <Image src={image} alt="" className="h-full object-cover" />
      <div className="h-full w-full px-5 flex items-center">{name}</div>
