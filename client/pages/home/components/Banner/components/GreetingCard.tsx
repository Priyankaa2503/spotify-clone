...
const GreetingCard: React.FC<GreetingCardProps> = ({ name, image }) => {
  ...
  return (
    <div
      className="bg-[#30303080] hover:bg-[#45454580] transition duration-300 flex h-20 tracking-wider font-semibold cursor-pointer w-full rounded-md overflow-hidden"
      onMouseEnter={() => setShowPlayButton(true)}
      onMouseLeave={() => setShowPlayButton(false)}
    >
      <Image src={image} alt="" className="h-full object-cover" />
      <div className="h-full w-[60%] px-5 flex items-center">{name}</div>
